...
public class DashboardPageTest extends Base {
	...
	public void verify_list_items_under_my_action_widget(String label1, String label2, String label3, String label4) {
		List<WebElement> itemsList = dpo.get_myactions_widget_list_items();
		for(int i=0; i<itemsList.size();i++) {
			String itemHeading = itemsList.get(i).getText().toString().trim();
			if(itemHeading.contains(label1) || itemHeading.contains(label2) || itemHeading.contains(label3) || itemHeading.contains(label4)) {
				Assert.assertTrue(true);
			}
		}
	}
	...
	public Object[][] getData3(){
		Object[][] data = new Object[1][4];
		data[0][0]="Leave Requests to Approve";
		data[0][1]="Timesheets to Approve";
		data[0][2]="Pending Self Review";
		data[0][3]="Candidate to Interview";
		return data;
	}
	
	
	@Test(dataProvider="quicklaunch_list")
	public void verify_list_items_under_quicklaunch_widget(String label1, String label2, String label3, String label4, String label5, String label6) {
		List<WebElement> list = dpo.get_quicklaunch_widget_list_items();
		for(int i=0; i<list.size();i++) {
			String heading = list.get(i).getText().toString().trim();
			if(heading.contains(label1) || heading.contains(label2) || heading.contains(label3) || heading.contains(label4) || heading.contains(label5) || heading.contains(label6)) {
				Assert.assertTrue(true);
			}
		}
	}
	
	@DataProvider(name="quicklaunch_list")
	public Object[][] getData4(){
		Object[][] data = new Object[1][6];
		data[0][0]="Assign Leave";
		data[0][1]="Leave List";
		data[0][2]="Timesheets";
		data[0][3]="Apply Leave";
		data[0][4]="My Leave";
		data[0][5]="My Timesheet";
		return data;
	}
